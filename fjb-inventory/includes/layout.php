...
<aside class="sidebar" id="sidebar">
  <div class="sidebar-brand">
    <img src="assets/img/fjb-logo.jpg" alt="FJB Logo">
  ...
  <nav class="sidebar-nav">
    ...
    <!-- Write Off Dropdown -->
    <div>
      <button onclick="toggleWriteoff()" id="writeoffToggle"
        style="width:100%;display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:8px;
          background:<?= in_array($active_nav,['writeoff','writeoff_collected']) ? 'rgba(242,140,40,.15)' : 'none' ?>;
          color:<?= in_array($active_nav,['writeoff','writeoff_collected']) ? 'var(--accent)' : '#94a3b8' ?>;
          border:none;font-size:13.5px;font-weight:<?= in_array($active_nav,['writeoff','writeoff_collected']) ? '600' : '500' ?>;
          cursor:pointer;font-family:inherit;margin-bottom:2px;transition:all .15s;text-align:left">
        <i class="bi bi-pen-fill" style="font-size:16px;width:20px;text-align:center;flex-shrink:0"></i>
        <span style="flex:1">Write Off</span>
        <?php
          $wo_pending_count = $db->query("SELECT COUNT(*) as c FROM ewaste_items WHERE disposal_status='Pending'")->fetch_assoc()['c'];
          if ($wo_pending_count > 0) echo '<span class="badge-count">'.$wo_pending_count.'</span>';
        ?>
        <i class="bi bi-chevron-down" id="writeoffChevron"
          style="font-size:11px;transition:transform .2s;<?= in_array($active_nav,['writeoff','writeoff_collected']) ? 'transform:rotate(180deg)' : '' ?>"></i>
      </button>
      <div id="writeoffMenu" style="<?= in_array($active_nav,['writeoff','writeoff_collected']) ? '' : 'display:none;' ?>padding-left:14px;margin-top:2px">
        <a href="writeoff.php" class="nav-link <?= $active_nav==='writeoff'?'active':'' ?>"
          style="padding:7px 12px;font-size:13px">
          <i class="bi bi-send-fill" style="font-size:14px"></i> Submit / Approve
          <?php if ($wo_pending_count > 0) echo '<span class="badge-count">'.$wo_pending_count.'</span>'; ?>
        </a>
        <a href="writeoff.php?view=collected" class="nav-link <?= $active_nav==='writeoff_collected'?'active':'' ?>"
          style="padding:7px 12px;font-size:13px">
          <i class="bi bi-folder-check" style="font-size:14px"></i> Collected Proofs
        </a>
      </div>
    </div>
