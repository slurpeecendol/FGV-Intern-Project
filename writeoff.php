...
// ── WRITE-OFF FORM EXCEL LINK (change this URL to your Excel/OneDrive/SharePoint link) ──
$WRITEOFF_FORM_URL = 'download_form.php';

// ── ADMIN APPROVE ALL WRITE-OFFS ──
if (isset($_GET['action']) && $_GET['action'] === 'approve_all_wo' && isAdmin()) {
    $all = $db->query("SELECT * FROM ewaste_items WHERE disposal_status='Pending'");
    while ($item = $all->fetch_assoc()) {
        ...
        logActivity($_SESSION['user_id'], 'APPROVE_WRITEOFF', 'ewaste', $wid, 'Bulk approved write-off: '.$item['description']);
    }
    header('Location: writeoff.php?msg=approved_all'); exit;
}
        ...
        foreach ($bulk_ids as $bid) {
            ...
            if ($item) {
                ...
                if (!$exists) {
                    ...
                    $stmt->bind_param('ssssisssssi',$an,$ac,$de,$sn,$bid,$wo_notes,$wo_name,$wo_desig,$wo_date,$wo_sid,$_SESSION['user_id']);
...
if ($url_msg === 'approved_all') $msg = 'All pending write-offs have been approved.';
if ($url_msg === 'rejected')  $msg = 'Write-off rejected. Item restored to IT Assets.';

$page_title = 'Write Off Authorisation';
$active_nav = 'writeoff';
  ...
  <form method="POST" enctype="multipart/form-data">
    ...
    <div class="row g-3 mb-4">
      ...
      <div class="col-md-6">
        ...
        <input type="time" name="writeoff_time" class="form-control" id="bulkWriteoffTime" value="<?= date('H:i') ?>">
      </div>
    </div>
    ...
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px">
      ...
      <a href="<?= h($WRITEOFF_FORM_URL) ?>" target="_blank"
        style="display:inline-flex;align-items:center;gap:5px;background:rgba(22,163,74,.1);color:#16a34a;border:1.5px solid rgba(22,163,74,.25);border-radius:7px;padding:4px 12px;font-size:12px;font-weight:700;text-decoration:none;font-family:'Plus Jakarta Sans',sans-serif;white-space:nowrap;margin-left:4px">
        <i class="bi bi-file-earmark-excel-fill"></i> Write-Off Form
      </a>
    </div>
    <div style="border:2px dashed var(--border);border-radius:10px;padding:24px;text-align:center;background:var(--body-bg);cursor:pointer;transition:border-color .2s;margin-bottom:8px"
      onclick="document.getElementById('bulkProofFile').click()"
      ondragover="event.preventDefault();this.style.borderColor='var(--accent)'"
      ondragleave="this.style.borderColor='var(--border)'"
      ondrop="event.preventDefault();this.style.borderColor='var(--border)';document.getElementById('bulkProofFile').files=event.dataTransfer.files;document.getElementById('bulkProofLabel').textContent=event.dataTransfer.files[0].name;bulkProofChanged()">
      ...
      <div style="font-size:12px;margin-top:4px"><span style="color:#dc2626;font-weight:700">Required</span> &mdash; <span style="color:var(--muted)">PDF, JPG, PNG</span></div>
    </div>
    <input type="file" id="bulkProofFile" name="writeoff_proof" accept=".pdf,.jpg,.jpeg,.png" required style="display:none"
      onchange="document.getElementById('bulkProofLabel').textContent=this.files[0]?.name||'Click or drag to upload proof document';bulkProofChanged()">
    <div id="bulkProofOk" style="display:none;font-size:12px;color:#16a34a;font-weight:600;margin-bottom:14px;font-family:'Plus Jakarta Sans',sans-serif">
      <i class="bi bi-check-circle-fill"></i> <span id="bulkProofName"></span>
    </div>

    <!-- Confirmation Checkbox -->
    <div style="background:rgba(248,81,73,.05);border:1.5px solid rgba(248,81,73,.2);border-radius:10px;padding:14px 16px;display:flex;align-items:flex-start;gap:12px;margin-bottom:20px;margin-top:8px">
      <input type="checkbox" id="bulkConfirmChk" style="width:17px;height:17px;accent-color:var(--red);cursor:pointer;flex-shrink:0;margin-top:2px" onchange="bulkCheckChanged()">
      <label for="bulkConfirmChk" style="cursor:pointer;font-size:13px;color:var(--text);margin:0;line-height:1.6;font-family:'Plus Jakarta Sans',sans-serif">
        I confirm that I authorise the write-off of <strong><?= count($bulk_items) ?> asset(s)</strong> for E-Waste disposal. This will be submitted to the admin for approval.
      </label>
    </div>

    <div style="display:flex;gap:10px;margin-top:8px">
      <button type="button" id="bulkSubmitBtn"
        onclick="bulkSubmitClicked(this.form || this.closest('form'))"
        style="background:var(--accent);color:#fff;border:none;border-radius:9px;padding:11px 24px;font-family:'Plus Jakarta Sans',sans-serif;font-size:14px;font-weight:700;display:flex;align-items:center;gap:8px;opacity:.45;cursor:not-allowed;pointer-events:none">
...
<script>
function bulkProofChanged() {
  const f = document.getElementById('bulkProofFile').files[0];
  const ok = document.getElementById('bulkProofOk');
  if (f) { ok.style.display='block'; document.getElementById('bulkProofName').textContent=f.name+' ('+(f.size/1024).toFixed(1)+' KB)'; }
  else { ok.style.display='none'; }
  updateBulkBtn();
}
function bulkCheckChanged() { updateBulkBtn(); }
function updateBulkBtn() {
  const hasFile = document.getElementById('bulkProofFile').files.length > 0;
  const checked = document.getElementById('bulkConfirmChk').checked;
  const btn = document.getElementById('bulkSubmitBtn');
  const ready = hasFile && checked;
  btn.style.opacity = ready ? '1' : '0.45';
  btn.style.cursor = ready ? 'pointer' : 'not-allowed';
  btn.style.pointerEvents = ready ? 'auto' : 'none';
}
function bulkSubmitClicked(form) {
  const hasFile = document.getElementById('bulkProofFile').files.length > 0;
  const checked = document.getElementById('bulkConfirmChk').checked;
  if (!hasFile) { alert('Please upload a proof document before submitting.'); return; }
  if (!checked) { alert('Please tick the confirmation checkbox before submitting.'); return; }
  form.submit();
}
</script>
<?php endif; ?>
...
<div class="table-card">
  <div style="padding:14px 20px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between">
    ...
    <div style="display:flex;align-items:center;gap:10px">
      ...
      <?php if (isAdmin()): ?>
      <a href="writeoff.php?action=approve_all_wo" onclick="return confirm('Approve all <?= $wo_count ?> pending write-off(s)? All items will move to E-Waste.')"
        style="display:inline-flex;align-items:center;gap:5px;background:#16a34a;color:#fff;border-radius:8px;padding:6px 16px;font-size:12px;font-weight:700;text-decoration:none;font-family:'Plus Jakarta Sans',sans-serif">
...
<script>
// Live clock — updates time fields every second unless user has manually changed them
function tickClock() {
  const now = new Date();
  const hh = String(now.getHours()).padStart(2,'0');
  const mm = String(now.getMinutes()).padStart(2,'0');
  const timeStr = hh + ':' + mm;
  ['writeoffTime','bulkWriteoffTime'].forEach(function(id) {
    const el = document.getElementById(id);
    if (el && !el.dataset.manual) el.value = timeStr;
  });
}
tickClock();
setInterval(tickClock, 1000);
// Stop auto-updating if user manually changes the field
document.addEventListener('change', function(e) {
  if (e.target.id === 'writeoffTime' || e.target.id === 'bulkWriteoffTime') {
